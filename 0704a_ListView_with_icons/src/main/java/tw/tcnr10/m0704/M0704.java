...
//
        for (int i = 0; i < listFromResource.length; i++) {
            ...
            String idName="img"+String.format("%02d"+"th",+i); //
            int resID = getResources().getIdentifier(idName,"drawable",getPackageName()); //靠圖片抓取
            item.put("imgView",resID);
            item.put("imgView", R.drawable.week);  //week=7days
